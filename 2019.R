...
# Abrir archivo CSV
df <- read.csv("datos finales 2019.csv", header = TRUE, sep = ";")[-1]  # Leer datos y omitir la primera columna
...
# Filtrar y mantener solo los registros con valores en la columna GRD2019
df$GRD2019 <- gsub(",", ".", df$GRD2019)  # Reemplazar comas por puntos
df$GRD2019 <- na_if(df$GRD2019, "")       # Reemplazar cadenas vacías por NA
df$GRD2019 <- as.numeric(df$GRD2019)      # Convertir a tipo numérico
df <- df[complete.cases(df$GRD2019), ]    # Eliminar registros con valores NA en GRD2019
...
predictors(rfe_result)
 ...
 save(rfe_result,file="rfe_result_rf_2019_grd242.RData")
...
for (n in 2:120) {
  ...
  # Agregar la variable dependiente 'GRD2019' al DataFrame
  df$GRD2019 <- GRD
  
  # Dividir los datos en conjuntos de entrenamiento y prueba
  set.seed(565)
  training.samples <- df$GRD2019 %>%
  ...
  RMSE = RMSE(y_pred = predictions, y_true = test.data$GRD2019)
  MAE = MAE(y_pred = predictions, y_true = test.data$GRD2019)
  R2 = R2_Score(y_pred = predictions, y_true = test.data$GRD2019)
...
correlacion_lista <- matrix(ncol = 0, nrow = 102)

for(i in c(1:102)){
  resultado <- cor(df[row.names(varImp(rfe_result))[i]], df$GRD2019)
