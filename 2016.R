...
# Abrir archivo CSV
df <- read.csv("datos finales 2016.csv", header = TRUE, sep = ";")[-1]  # Leer datos y omitir la primera columna
...
# Filtrar y mantener solo los registros con valores en la columna GRD2016
df$GRD2016 <- gsub(",", ".", df$GRD2016)  # Reemplazar comas por puntos
df$GRD2016 <- na_if(df$GRD2016, "")       # Reemplazar cadenas vacías por NA
df$GRD2016 <- as.numeric(df$GRD2016)      # Convertir a tipo numérico
df <- df[complete.cases(df$GRD2016), ]    # Eliminar registros con valores NA en GRD2016
...
predictors(rfe_result)
 ...
 # save(rfe_result,file="rfe_result_rf_2016_grd10.RData")
# load('rfe_result_rf_2016_grd10.RData')
...
for (n in 2:17) {
  ...
  # Agregar la variable dependiente 'GRD2016' al DataFrame
  df$GRD2016 <- GRD
  
  # Dividir los datos en conjuntos de entrenamiento y prueba
  set.seed(565)
  training.samples <- df$GRD2016 %>%
  ...
  RMSE = RMSE(y_pred = predictions, y_true = test.data$GRD2016)
  MAE = MAE(y_pred = predictions, y_true = test.data$GRD2016)
  R2 = R2_Score(y_pred = predictions, y_true = test.data$GRD2016)
...
correlacion_lista <- matrix(ncol = 0, nrow = 17)

for(i in c(1:17)){
  resultado <- cor(df[row.names(varImp(rfe_result))[i]], df$GRD2016)
