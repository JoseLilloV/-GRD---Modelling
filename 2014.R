...
# Abrir archivo CSV
df <- read.csv("datos finales 2014.csv", header = TRUE, sep = ";")[-1]  # Leer datos y omitir la primera columna
...
# Filtrar y mantener solo los registros con valores en la columna GRD2014
df$GRD2014 <- gsub(",", ".", df$GRD2014)  # Reemplazar comas por puntos
df$GRD2014 <- na_if(df$GRD2014, "")       # Reemplazar cadenas vacías por NA
df$GRD2014 <- as.numeric(df$GRD2014)      # Convertir a tipo numérico
df <- df[complete.cases(df$GRD2014), ]    # Eliminar registros con valores NA en GRD2014
...
# save(rfe_result,file="rfe_result_rf_2014_grd10.RData")
...
for (n in 2:120) {
  ...
  df <- select(df_temp, row.names(varImp(rfe_result2))[1:n])
  
  # Agregar la variable dependiente 'GRD2014' al DataFrame
  df$GRD2014 <- GRD
  
  # Dividir los datos en conjuntos de entrenamiento y prueba
  set.seed(565)
  training.samples <- df$GRD2014 %>%
  ...
  set.seed(565)
  rf <- train(
    x = select(train.data, -GRD2014),
    y = train.data$GRD2014,
    tuneGrid = Grid,
  ...
  RMSE = RMSE(y_pred = predictions, y_true = test.data$GRD2014)
  MAE = MAE(y_pred = predictions, y_true = test.data$GRD2014)
  R2 = R2_Score(y_pred = predictions, y_true = test.data$GRD2014)
...
correlacion_lista <- as.dataframe(matrix(ncol = 2, nrow = 22))

for(i in c(1:22)){
  resultado <- cor(df[row.names(varImp(rfe_result2))[i]], df$GRD2014)
