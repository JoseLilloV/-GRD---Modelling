...
# Abrir archivo CSV
df <- read.csv("datos finales 2015.csv", header = TRUE, sep = ";")[-1]  # Leer datos y omitir la primera columna
...
# Filtrar y mantener solo los registros con valores en la columna GRD2015
df$GRD2015 <- gsub(",", ".", df$GRD2015)  # Reemplazar comas por puntos
df$GRD2015 <- na_if(df$GRD2015, "")       # Reemplazar cadenas vacías por NA
df$GRD2015 <- as.numeric(df$GRD2015)      # Convertir a tipo numérico
df <- df[complete.cases(df$GRD2015), ]    # Eliminar registros con valores NA en GRD2015
...
predictors(rfe_result)
 ...
 save(rfe_result,file="rfe_result_rf_2015_grd208.RData")
...
for (n in 2:120) {
  ...
  # Agregar la variable dependiente 'GRD2015' al DataFrame
  df$GRD2015 <- GRD
  
  # Dividir los datos en conjuntos de entrenamiento y prueba
  set.seed(565)
  training.samples <- df$GRD2015 %>%
  ...
  RMSE = RMSE(y_pred = predictions, y_true = test.data$GRD2015)
  MAE = MAE(y_pred = predictions, y_true = test.data$GRD2015)
  R2 = R2_Score(y_pred = predictions, y_true = test.data$GRD2015)
...
for (n in 2:120) {
  ...
  # Seleccionar las características (columnas) en df_temp basadas en la importancia de características resultante del RFE (selecciona las n características más importantes)
  df <- select(df, row.names(varImp(rfe_result))[1:22])
...
correlacion_lista <- matrix(ncol = 0, nrow = 14)

for(i in c(1:14)){
  resultado <- cor(df[row.names(varImp(rfe_result))[i]], df$GRD2015)
