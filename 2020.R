...
# Abrir archivo CSV
df <- read.csv("datos finales 2020.csv", header = TRUE, sep = ";")[-1]  # Leer datos y omitir la primera columna
...
# Filtrar y mantener solo los registros con valores en la columna GRD2020
df$GRD2020 <- gsub(",", ".", df$GRD2020)  # Reemplazar comas por puntos
df$GRD2020 <- na_if(df$GRD2020, "")       # Reemplazar cadenas vacías por NA
df$GRD2020 <- as.numeric(df$GRD2020)      # Convertir a tipo numérico
df <- df[complete.cases(df$GRD2020), ]    # Eliminar registros con valores NA en GRD2020
...
save(rfe_result,file="rfe_result_rf_2020_grd975.RData")
...
for (n in 2:120) {
  ...
  # Agregar la variable dependiente 'GRD2020' al DataFrame
  df$GRD2020 <- GRD
  
  # Dividir los datos en conjuntos de entrenamiento y prueba
  set.seed(565)
  training.samples <- df$GRD2020 %>%
  ...
  RMSE = RMSE(y_pred = predictions, y_true = test.data$GRD2020)
  MAE = MAE(y_pred = predictions, y_true = test.data$GRD2020)
  R2 = R2_Score(y_pred = predictions, y_true = test.data$GRD2020)
...
correlacion_lista <- matrix(ncol = 0, nrow = 104)

for(i in c(1:104)){
  resultado <- cor(df[row.names(varImp(rfe_result))[i]], df$GRD2020)
