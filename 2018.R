...
# Abrir archivo CSV
df <- read.csv("datos finales 2018.csv", header = TRUE, sep = ";")[-1]  # Leer datos y omitir la primera columna
...
# Filtrar y mantener solo los registros con valores en la columna GRD2018
df$GRD2018 <- gsub(",", ".", df$GRD2018)  # Reemplazar comas por puntos
df$GRD2018 <- na_if(df$GRD2018, "")       # Reemplazar cadenas vacías por NA
df$GRD2018 <- as.numeric(df$GRD2018)      # Convertir a tipo numérico
df <- df[complete.cases(df$GRD2018), ]    # Eliminar registros con valores NA en GRD2018
...
predictors(rfe_result)
 ...
 save(rfe_result,file="rfe_result_rf_2017_grd22.RData")
...
for (n in 2:51) {
  ...
  # Agregar la variable dependiente 'GRD2018' al DataFrame
  df$GRD2018 <- GRD
  
  # Dividir los datos en conjuntos de entrenamiento y prueba
  set.seed(565)
  training.samples <- df$GRD2018 %>%
  ...
  RMSE = RMSE(y_pred = predictions, y_true = test.data$GRD2018)
  MAE = MAE(y_pred = predictions, y_true = test.data$GRD2018)
  R2 = R2_Score(y_pred = predictions, y_true = test.data$GRD2018)
...
correlacion_lista <- matrix(ncol = 0, nrow = 49)

for(i in c(1:49)){
  resultado <- cor(df[row.names(varImp(rfe_result))[i]], df$GRD2018)
